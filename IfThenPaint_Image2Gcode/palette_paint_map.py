...
def palette_paint_map(layer_paint_distance, machine_objects, paints, tool_profiles):
    ...
    for layer in layer_paint_distance:
        ...
        paint_volume = paint_bead_distance/10*(dispenser['paint_bead_width']/10)*(dispenser['paint_bead_height']/10)
        dispenser_paint_volume.append({'paint_name': paint_name, 
    ...
    return palette_paint_map, dispenser_paint_volume
...
if __name__ == '__main__':
    ...
    palette_paint_map, dispenser_paint_volume = palette_paint_map(layer_paint_distance,
                                                                  machine_objects,
                                                                  paints,
                                                                  tool_profiles)
    
    with open(os.path.join(DATA_PATH, 'dispenser_paint_volume.txt'), 'w') as f:
        json.dump(dispenser_paint_volume, f, separators = (',', ':'), sort_keys = True, indent = 4)
    f.close()
